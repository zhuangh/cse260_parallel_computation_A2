...
k  
  ...
  */
	...
	if( N % BLOCKDIM_X  ==0 ){
		...
		As( (ty + BLOCK_SIZE/2) , tx) = A[ a+ wA *(ty+ BLOCK_SIZE/2) +tx];
		Bs( (ty + BLOCK_SIZE/2), tx) = B[ b+ wB *(ty+ BLOCK_SIZE/2) +tx];
		__syncthreads();
...
#pragma unroll
		for (int k = 0 ; k < BLOCK_SIZE; k++)
		{
		    ...
		    Csub[1] += As(ty + BLOCK_SIZE/2 ,k) * Bs(k, tx)  ;
	    ...
	    C[c+ wB *ty + tx] =Csub[0];
	    C[c+ wB *(ty+ BLOCK_SIZE/2) + tx] = Csub[1];
	}
	else{ 
#ifdef VOLV
	    int aBegin = wA * BLOCK_SIZE * by;
